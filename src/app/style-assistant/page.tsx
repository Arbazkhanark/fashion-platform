...
//   const occasions = ["Wedding", "Party", "Office", "Casual", "Special Event", "Date Night"]
//   const bodyTypes = ["Petite", "Average", "Tall", "Curvy", "Athletic", "Pear-shaped"]
//   const skinTones = ["Fair", "Medium", "Olive", "Deep", "Warm", "Cool"]
//   const budgets = ["Budget (₹5K-15K)", "Mid-range (₹15K-50K)", "Premium (₹50K-1L)", "Luxury (₹1L+)"]
//   const styles = ["Classic", "Trendy", "Bohemian", "Minimalist", "Glamorous", "Casual", "Ethnic", "Fusion"]
//   const colors = ["Neutrals", "Pastels", "Bright", "Earth Tones", "Jewel Tones", "Monochrome"]
//   const concerns = ["Comfort", "Durability", "Sustainability", "Uniqueness", "Versatility", "Confidence"]
...
//     setPreferences((prev) => ({
//       ...prev,
//       style: prev.style.includes(style) ? prev.style.filter((s) => s !== style) : [...prev.style, style],
//     }))
//   }
...
//         reason: "Perfect for your wedding styling needs with 12 years of experience in bridal wear",
//         match: 95,
//       })
//     }

//     if (preferences.occasion.includes("Party") || preferences.occasion.includes("Special Event")) {
//       mockRecommendations.push({
//         expertId: "3",
//         expertName: "Neha Patel",
//         title: "Makeup Artist",
//         reason: "Expert in party makeup with glamorous looks matching your style preferences",
//         match: 92,
//       })
//     }
...
//         reason: "Specializes in professional and minimalist styles perfect for office wear",
//         match: 88,
//       })
//     }
...
//         title: "Hair Stylist",
//         reason: "Expert in casual styling and modern haircuts for everyday looks",
//         match: 85,
//       })
//     }
...
//             <h1 className="text-4xl font-bold">AI Style Assistant</h1>
//           </div>
//           <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
//             Get personalized recommendations for fashion designers and beauty experts based on your style preferences
//           </p>
//         </div>
...
//                           onCheckedChange={() => setPreferences((prev) => ({ ...prev, bodyType: type }))}
//                         />
...
//                           onCheckedChange={() => setPreferences((prev) => ({ ...prev, skinTone: tone }))}
//                         />
//                         <label htmlFor={`skin-${tone}`} className="text-sm cursor-pointer">
//                           {tone}
//                         </label>
//                       </div>
//                     ))}
...
//                         onCheckedChange={() => setPreferences((prev) => ({ ...prev, budget }))}
//                       />
//                       <label htmlFor={`budget-${budget}`} className="text-sm cursor-pointer">
//                         {budget}
//                       </label>
//                     </div>
//                   ))}
...
//                 <CardDescription>Select all that resonate with you</CardDescription>
//               </CardHeader>
//               <CardContent>
...
//                       <Checkbox
//                         id={`concern-${concern}`}
//                         checked={preferences.concerns.includes(concern)}
...
//                 onClick={() => setStep(2)}
//                 disabled={!preferences.occasion.length || !preferences.bodyType || !preferences.budget}
//               >
//                 Next
//               </Button>
//             </div>
...
//               <CardDescription>Any specific requirements or questions?</CardDescription>
//             </CardHeader>
...
//                   Back
//                 </Button>
//                 <Button className="flex-1 gap-2" onClick={generateRecommendations} disabled={loading}>
//                   {loading ? (
//                     <>
...
//                       Based on your preferences, here are the best-matched experts for you:
//                     </p>
//                   </div>
//                 </div>
//               </CardContent>
...
//                   const expert = mockExperts.find((e) => e.id === rec.expertId)
//                   return (
//                     <Card key={rec.expertId} className="overflow-hidden hover:shadow-lg transition-smooth">
...
//                         <div>
//                           <h3 className="font-semibold text-lg">{rec.expertName}</h3>
...
//                   <p className="text-muted-foreground">No recommendations found. Please try different preferences.</p>
//                 </CardContent>
//               </Card>
//             )}
...
import { Textarea } from "@/components/ui/textarea"
import { Checkbox } from "@/components/ui/checkbox"
import { Sparkles, Loader, ArrowRight, Star, Users, Palette, Zap, Brain, Wand2 } from "lucide-react"
import Link from "next/link"
import { mockExperts } from "@/lib/mock-data"
...
export default function StyleAssistantPage() {
  ...
  const generateFashionAdvice = (prefs: StylePreferences): FashionAdvice => {
    const advice: FashionAdvice = {
      ...
      description: "Based on your preferences, here's what would work best for you:",
      tips: [],
      colors: [],
      styles: []
    }
    ...
    if (prefs.bodyType === "Petite") {
      advice.tips.push("Opt for vertical lines and monochrome outfits to create height illusion")
      advice.tips.push("Choose fitted clothing that doesn't overwhelm your frame")
      ...
      advice.tips.push("Layered looks and statement accessories complement your height")
    }
  ...
  const generateRecommendations = async () => {
    ...
    if (preferences.occasion.includes("Wedding")) {
      mockRecommendations.push({
        ...
        reason: "Perfect for your wedding styling needs with 12 years of experience in bridal wear",
        match: 95,
        aiAdvice: "Specializes in custom bridal wear that complements your body type and skin tone"
      })
    }
    ...
    if (preferences.occasion.includes("Party") || preferences.occasion.includes("Special Event")) {
      mockRecommendations.push({
        ...
        reason: "Expert in party makeup with glamorous looks matching your style preferences",
        match: 92,
        aiAdvice: "Creates stunning party looks that enhance your natural features"
      })
    }
    ...
    if (preferences.occasion.includes("Office")) {
      mockRecommendations.push({
        ...
        reason: "Specializes in professional and minimalist styles perfect for office wear",
        match: 88,
        aiAdvice: "Designs office wear that balances professionalism with personal style"
      })
    }
    ...
    if (mockRecommendations.length < 3) {
      mockRecommendations.push({
        ...
        reason: "Great for versatile styling options matching your preferences",
        match: 82,
        aiAdvice: "Versatile designer who can adapt to multiple style preferences"
      })
    }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className={`text-center mb-12 transition-all duration-1000 ${isVisible ? 'animate-glow-in' : 'opacity-0'}`}>
            ...
            <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
              Get personalized fashion advice and expert recommendations powered by artificial intelligence
            </p>
          </div>
          ...
          {step === 1 && (
            <div className="space-y-6 animate-slide-up">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {occasions.map((occasion, index) => (
                      ...
                      >
                        ...
                        />
                        <label htmlFor={`occasion-${occasion}`} className="text-sm cursor-pointer text-gray-700 dark:text-gray-300 font-medium">
                          {occasion}
                        </label>
                      </div>
                    ))}
              ...
              {/* Body Type & Skin Tone */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-xl animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
                  <CardHeader className="pb-4">
                    <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {bodyTypes.map((type, index) => (
                        <div key={type} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                          <Checkbox
                            id={`body-${type}`}
                            checked={preferences.bodyType === type}
                            onCheckedChange={() => setPreferences((prev) => ({ ...prev, bodyType: type }))}
                          ...
                          />
                          <label htmlFor={`body-${type}`} className="text-sm cursor-pointer text-gray-700 dark:text-gray-300">
                            {type}
                          </label>
                        </div>
                      ))}
                ...
                <Card className="border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-xl animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
                  <CardHeader className="pb-4">
                    <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {skinTones.map((tone, index) => (
                        <div key={tone} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                          <Checkbox
                            id={`skin-${tone}`}
                            checked={preferences.skinTone === tone}
                            onCheckedChange={() => setPreferences((prev) => ({ ...prev, skinTone: tone }))}
                          ...
                          />
                          <label htmlFor={`skin-${tone}`} className="text-sm cursor-pointer text-gray-700 dark:text-gray-300">
                            {tone}
                          </label>
                        </div>
                      ))}
              ...
              {/* Budget */}
              <Card className="border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-xl animate-fade-in-up" style={{ animationDelay: "0.4s" }}>
                <CardHeader className="pb-4">
                  <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {budgets.map((budget, index) => (
                      <div key={budget} className="flex items-center gap-3 p-3 rounded-lg bg-white/50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 hover:border-green-300 dark:hover:border-green-500 transition-all duration-300 hover:scale-105">
                        <Checkbox
                          id={`budget-${budget}`}
                          checked={preferences.budget === budget}
                          onCheckedChange={() => setPreferences((prev) => ({ ...prev, budget }))}
                        ...
                        />
                        <label htmlFor={`budget-${budget}`} className="text-sm cursor-pointer text-gray-700 dark:text-gray-300 font-medium">
                          {budget}
                        </label>
                      </div>
                    ))}
              ...
              {/* Style Preferences */}
              <Card className="border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-xl animate-fade-in-up" style={{ animationDelay: "0.5s" }}>
                <CardHeader className="pb-4">
                  <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
                    <Wand2 className="w-5 h-5 text-purple-500" />
                    Style Preferences
                  </CardTitle>
                  <CardDescription className="text-gray-600 dark:text-gray-400">Select all that resonate with you</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {styles.map((style, index) => (
                      <div key={style} className="flex items-center gap-3 p-3 rounded-lg bg-white/50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 hover:border-purple-300 dark:hover:border-purple-500 transition-all duration-300 hover:scale-105">
                        <Checkbox
                          id={`style-${style}`}
                          checked={preferences.style.includes(style)}
                          onCheckedChange={() => handleStyleToggle(style)}
                        ...
                        />
                        <label htmlFor={`style-${style}`} className="text-sm cursor-pointer text-gray-700 dark:text-gray-300">
                          {style}
                        </label>
                      </div>
                    ))}
              ...
              {/* Navigation */}
              <div className="flex gap-3 pt-6 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
                <Link href="/" className="flex-1">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <h3 className="font-bold text-xl text-gray-900 dark:text-white">AI Style Analysis</h3>
                ...
                {recommendations.length > 0 ? (
                    ...
                    {recommendations.map((rec, index) => {
                      ...
                      return (
                        <Card 
                          ...
                          style={{ animationDelay: `${index * 0.1}s`, animationDuration: '8s' }}
                        >
                          ...
                          <CardContent className="pt-6 space-y-4">
                            <div>
                              <h3 className="font-bold text-xl text-gray-900 dark:text-white mb-1">{rec.expertName}</h3>
                              <p className="text-rose-600 dark:text-rose-400 font-medium">{rec.title}</p>
                            </div>

                            <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{rec.reason}</p>

                            <div className="bg-rose-50 dark:bg-rose-900/20 rounded-xl p-4">
                              <p className="text-sm text-rose-700 dark:text-rose-300 font-medium">AI Insight:</p>
                              <p className="text-sm text-rose-600 dark:text-rose-400 mt-1">{rec.aiAdvice}</p>
                            </div>
                  ...
                  <Card className="border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-xl">
                    <CardContent className="pt-6 text-center py-12">
                      ...
                      <p className="text-gray-600 dark:text-gray-400 text-lg">No recommendations found. Please try different preferences.</p>
                    </CardContent>
                  </Card>
                )}
