...
export default function CategoryGrid() {
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-white via-rose-50/20 to-amber-50/10 dark:from-gray-900 dark:via-rose-900/10 dark:to-amber-900/5 relative overflow-hidden">
      ...
      <div
        className="absolute top-40 right-20 w-1 h-1 bg-blue-400 rounded-full animate-float-particle opacity-40"
        style={{ animationDelay: "1s" }}
      />
      <div
        className="absolute bottom-32 left-1/4 w-1.5 h-1.5 bg-amber-400 rounded-full animate-float-particle opacity-50"
        style={{ animationDelay: "2s" }}
      />
      <div
        className="absolute top-1/2 right-1/3 w-1 h-1 bg-purple-400 rounded-full animate-float-particle opacity-30"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto leading-relaxed animate-fade-in-up">
            Discover curated fashion professionals specialized in your style
            needs
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          {categories.map((category, index) => {
            ...
            return (
              ...
              >
                <Card
                  className="h-full hover:shadow-2xl transition-all duration-700 cursor-pointer overflow-hidden border-0 bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl relative 
                ...
                >
                  {/* Animated Border */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-r ${category.color} rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                  >
                  ...
                  <div
                    className={`absolute -top-4 -right-4 w-20 h-20 bg-gradient-to-br ${category.color} rounded-full opacity-5 group-hover:opacity-10 transition-all duration-1000 group-hover:scale-150`}
                  />
                  ...
                  {/* Hover Glow Effect */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-r ${category.color} opacity-0 group-hover:opacity-10 transition-opacity duration-500 rounded-xl blur-xl`}
                  />
        ...
        >
          <div className="inline-flex items-center gap-3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-2xl px-6 py-4 shadow-lg hover:shadow-xl transition-all duration-500 hover:scale-105 group">
            ...
            <p className="text-gray-700 dark:text-gray-300 font-medium">
              ...
              <Link
                href="/designers"
                className="text-rose-600 dark:text-rose-400 hover:underline font-semibold group-hover:tracking-wide transition-all"
              >
